...
DOCUMENT_STOP_WORDS = {
    "and", "copy", "document", "for", "last", "months",
    "of", "size", "the", "years", "a", "an", "or",
    "with", "valid", "self", "attested", "recent", "original",
}
...
class DocumentIntelligence:
    ...
    def validate_upload(
        *,
        ...
        # Hard block: unsupported file type
        if not file_type_supported(file_name, content_type):
            errors.append({"message": f"Unsupported file type: {content_type or Path(file_name).suffix or 'unknown'}. Please upload PDF, JPG, PNG, or DOCX.", "source": "system"})

        # Hard block: file too small / blank
        if file_size and file_size < 5 * 1024:
            errors.append({"message": "File too small or blank. Please upload a clear, complete document.", "source": "system"})

        # Soft warn: document not in scheme requirements (don't fail — scheme data may be incomplete)
        if not matched_requirement:
            warnings.append({"message": f"'{selected_document}' is not listed as required for this scheme. Please verify with the scheme portal.", "source": "scheme"})

        # Filename check: only warn (never error) — generic filenames like IMG_1234.jpg are common
        file_doc_type = canonical_document_name(file_name)
        selected_doc_type = canonical_document_name(selected_document)
        if file_doc_type and selected_doc_type and file_doc_type != selected_doc_type:
            warnings.append({
                "message": f"Filename suggests '{file_doc_type.title()}' but '{selected_doc_type.title()}' was selected. Please verify the correct file was uploaded.",
                "source": "filename",
            })

        # Confidence score — give benefit of the doubt
        if errors:
            confidence = 10
        elif warnings:
            confidence = 70  # Reasonable confidence even with warnings
        else:
            confidence = 88
        ...
        # Verdict
        if errors:
            verdict, status, is_valid = "invalid", "error", False
        else:
            # No hard errors — accept the document (warnings are advisory only)
            verdict, status, is_valid = "valid", "valid", True

        scheme_name = scheme.get("scheme_name") or scheme.get("Scheme_Name") or "Selected Scheme"
        summary = (
            f"{selected_document} accepted for {scheme_name}."
            if is_valid
        ...
        return {
            ...
            "errors": errors,
            "warnings": warnings,
            "validationMode": "KARIOS Rule-Based Validation",
            "scheme": {
...
# ─── FREE AI FALLBACK FOR DOCUMENT VALIDATION ─────────────────────────────────
# Gemini → Groq → NVIDIA  (tries each in order, uses first that works)

def _build_doc_prompt(doc_name: str, scheme: dict, file_name: str, file_size: int, is_image: bool) -> str:
    return f"""You are an Indian government document verification officer.

DOCUMENT EXPECTED : "{doc_name}"
SCHEME            : "{scheme.get('scheme_name', scheme.get('name', ''))}" ({scheme.get('state', 'India')})
FILE              : {file_name or 'unknown'} ({f"{file_size/1024:.1f}KB" if file_size else '?'})
{"Examine the uploaded document image carefully." if is_image else "Non-image file: validate from filename and context only."}

VERDICT RULES — choose exactly one:
• "valid"    → Correct document type + genuine + readable
• "mismatch" → Completely wrong document type uploaded
• "invalid"  → Correct type but blurry / tampered / SAMPLE stamp / key data missing

FORMAT RULES:
• "errors": max ONE item, under 60 chars (e.g. "Aadhaar number not visible")
• "warnings": EMPTY [] unless confirmed non-critical issue
• "summary": max 8 words (e.g. "Aadhaar Card verified successfully")
• Do NOT write "based on filename" or "passes preliminary validation"

Reply ONLY with valid JSON, no markdown fences:
{{
  "verdict": "valid or mismatch or invalid",
  "valid": true or false,
  "documentType": "detected document type",
  "govBody": "issuing authority",
  "extractedFields": {{
    "name": "or null", "number": "or null",
    "dob": "DD/MM/YYYY or null", "state": "or null",
    "issueDate": "or null", "expiryDate": "or null"
  }},
  "errors": [],
  "warnings": [],
  "confidenceScore": 0,
...
def _parse_ai_json(text: str) -> dict:
    ...
    try:
        clean = _re.sub(r"^```json\s*", "", text, flags=_re.I)
        clean = _re.sub(r"^```\s*",    "", clean, flags=_re.I)
        clean = _re.sub(r"\s*```$",    "", clean).strip()
        return _json.loads(clean)
    except Exception:
        logger.error(f"AI JSON parse failed: {text[:200]}")
        return {
            "verdict": "invalid", "valid": False,
            "errors": ["AI response could not be parsed. Please retry."],
            "warnings": [], "confidenceScore": 0,
            "summary": "Retry validation.", "extractedFields": {},
        }


async def _call_gemini(image_b64, mime_type: str, prompt: str) -> str:
    import asyncio
    if not genai_active:
    ...
    if image_b64 and mime_type.startswith("image/"):
        parts.append({"inline_data": {"data": image_b64, "mime_type": mime_type}})
    parts.append(prompt)
    for attempt in range(3):
        try:
            ...
            if "429" in msg or "quota" in msg or "Too Many Requests" in msg:
                import re as _re
                match = _re.search(r"retry[^\d]*(\d+)", msg, _re.I)
                wait  = (int(match.group(1)) + 2) if match else (attempt + 1) * 20
                logger.warning(f"Gemini rate limited. Retry {attempt+1}/2 in {wait}s…")
                await asyncio.sleep(wait)
            else:
                raise


async def _call_groq(image_b64, mime_type: str, prompt: str) -> str:
    ...
    async with _httpx.AsyncClient(timeout=60) as c:
        r = await c.post(
            ...
            json={"model": "llama-3.2-11b-vision-preview", "max_tokens": 700,
                  "temperature": 0.1, "messages": [{"role": "user", "content": content}]},
        )
        r.raise_for_status()
        return r.json()["choices"][0]["message"]["content"]


async def _call_nvidia(image_b64, mime_type: str, prompt: str) -> str:
    import httpx as _httpx
    ...
    async with _httpx.AsyncClient(timeout=60) as c:
        r = await c.post(
            ...
            json={"model": "thudm/glm-4v-9b", "max_tokens": 700,
                  "temperature": 0.1, "messages": [{"role": "user", "content": content}]},
        )
        r.raise_for_status()
        return r.json()["choices"][0]["message"]["content"]
...
async def _ai_validate_with_fallback(image_b64, mime_type: str, prompt: str) -> str:
    """Tries Gemini → Groq → NVIDIA. Returns first success."""
    providers = [
        ("Gemini", _call_gemini,  genai_active),
        ("Groq",   _call_groq,    bool(os.getenv("GROQ_API_KEY"))),
        ("NVIDIA", _call_nvidia,  bool(os.getenv("NVIDIA_API_KEY"))),
    ]
    last_err = None
    for name, fn, available in providers:
        if not available:
            continue
        try:
            result = await fn(image_b64, mime_type, prompt)
            logger.info(f"✅ Doc validated via {name}")
            return result
        except Exception as e:
            logger.warning(f"⚠️  {name} failed: {str(e)[:80]}")
            last_err = e
    raise Exception(f"All AI providers failed. Last error: {last_err}")


# ─── MAIN UPLOAD + AI VALIDATION ENDPOINT ────────────────────────────────────
...
async def validate_doc_upload(
    ...
    image_b64 = _b64.b64encode(file_bytes).decode() if is_image else None

    # Reject tiny/empty files immediately
    if file_size < 5 * 1024:
        return {
            "success": False, "status": "error", "isValid": False,
            "errors": [{"message": "File too small — likely blank or corrupt.", "source": "system"}],
            "warnings": [], "summary": "File rejected.",
            "processingMs": int((_time.time() - t0) * 1000),
        }

    # Try AI validation with fallback
    prompt = _build_doc_prompt(doc_name, scheme_payload, file_name, file_size, is_image)
    try:
        raw_text = await _ai_validate_with_fallback(image_b64, mime_type, prompt)
        ai = _parse_ai_json(raw_text)

        # Normalise verdict → valid boolean
        if ai.get("verdict") == "valid":    ai["valid"] = True
        if ai.get("verdict") == "mismatch": ai["valid"] = False
        if ai.get("verdict") == "invalid":  ai["valid"] = False

        # Filter noisy AI prose errors
        ai_errors = [e for e in (ai.get("errors") or [])
                     if isinstance(e, str) and 0 < len(e) < 120
                     and "based on the filename" not in e.lower()
                     and "passes" not in e.lower()
                     and "appears to be" not in e.lower()
                     and "preliminary" not in e.lower()]
        ai_warns = [w for w in (ai.get("warnings") or []) if isinstance(w, str) and w.strip()]

        errors   = [{"message": m, "source": "ai"} for m in ai_errors]
        warnings = [{"message": m, "source": "ai"} for m in ai_warns]

        verdict = ai.get("verdict", "invalid")
        if   verdict == "mismatch":               status, is_valid = "error", False
        elif verdict == "valid" and not errors:   status, is_valid = "valid", True
        elif errors:                              status, is_valid = "error", False
        elif warnings:                            status, is_valid = "warn",  False
        else:
            is_valid = ai.get("valid", False)
            status   = "valid" if is_valid else "error"

        summary = ai.get("summary") or (
            "Document verified successfully." if is_valid else
            f"Wrong document. Expected: {doc_name}" if verdict == "mismatch" else
            (errors[0]["message"] if errors else "Document validation failed.")
        )

        ms = int((_time.time() - t0) * 1000)
        logger.info(f"[validate] {doc_name} → {status} | {verdict} | conf:{ai.get('confidenceScore', 0)} ({ms}ms)")

        return {
            "success": True, "status": status, "isValid": is_valid,
            "verdict":         verdict,
            "documentType":    ai.get("documentType", doc_name),
            "govBody":         ai.get("govBody", ""),
            "extractedFields": ai.get("extractedFields", {}),
            "errors":          errors,
            "warnings":        warnings,
            "confidenceScore": ai.get("confidenceScore", 0),
            "summary":         summary,
            "fileName":        file_name,
            "processingMs":    ms,
        }
        ...
        # All AI providers failed → fall back to rule-based validation
        logger.error(f"AI validation failed, using rule-based fallback: {e}")
        result = DocumentIntelligence.validate_upload(
            file_name=file_name,
            file_size=file_size,
        ...
        result["processingMs"] = int((_time.time() - t0) * 1000)
        return result


# Alias — some frontends call /v1/validate/document instead
...
async def validate_document_alias(
    ...
    return await validate_doc_upload(
        file=file, doc_name=docName, scheme=scheme, language=language
    )
