...
DOCUMENT_PROFILES: Dict[str, Dict[str, Any]] = {
    ...
    "bank statement": {
        ...
        "required_keywords": ["account", "ifsc"],
        "any_of_keywords": [["transaction", "balance", "debit", "credit", "bank"]],
        "banned_keywords": [
    ...
    "passport photo": {
        "aliases": ["passport photo", "passport size photo", "photograph", "photo"],
        "required_keywords": [],
        "any_of_keywords": [],
        "banned_keywords": [],
        "scoring_keywords": [],
...
def _score_ocr(ocr_text: str, profile: Dict, doc_name: str) -> Tuple[bool, List[Dict], Dict, int]:
    ...
    if required:
        ...
        if not found:
            # Soft-fail: OCR may have missed text in scanned/Tamil docs
            # Return special sentinel so caller can defer to AI instead of rejecting
            return False, [{
                "message": (
                ...
                "source": "layer2_required_keyword_defer_ai", "layer": "2",
            }], {}, 0
...
def layer2_ocr_gate(
    ...
    # Face detection for identity documents
    if profile.get("face_required") and ocr_img:
        face_count = _detect_faces(ocr_img)
        if face_count == 0:
            errors.append({
                "message": (
                    f"No human face detected in the '{doc_name}'. "
                    "Please upload a clear photo-bearing identity document."
                ),
                "source": "layer2_face_detection", "layer": "2",
            })
            return False, errors, extracted, score, ocr_text
        if face_count > 1:
    ...
    if profile is None:
        ...
        if not ocr_empty:
            if not l2_passed:
                ...
                # If failure source is "defer_ai", skip rejection and let AI decide
                if "defer_ai" in src:
                    add_step("keyword_check", label, "skipped",
                             "OCR inconclusive — AI vision will verify")
                elif "face" in src:
                ...
                if "defer_ai" not in src:
                    ...
                    return await localize_validation_payload(result, lang_code) if localize_validation_payload else result
                # else: fall through to Layer 3 AI
            else:
        ...
        try:
            ...
            if verdict == "mismatch":
                ...
                result = build_response(
                    ...
                    gov_body=str(ai.get("govBody") or ""), ocr_score=ocr_score,
                )
