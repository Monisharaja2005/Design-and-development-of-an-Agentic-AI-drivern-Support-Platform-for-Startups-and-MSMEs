...
async def validate_doc_upload(
    ...
    # Layer 1: File sanity  (format, size, corruption)       — no API cost
    # Layer 2: OCR + Regex  (keywords, patterns, face check) — CPU only
    # Layer 3: AI Vision    (Gemini/Groq/NVIDIA)             — only if L1+L2 pass
    # Wrong/irrelevant documents are rejected at Layer 2 with a clear error
    # message — zero AI API tokens consumed for those cases.
    # ─────────────────────────────────────────────────────────────────────────
